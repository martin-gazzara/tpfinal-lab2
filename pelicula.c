...
//Verifica si una Pel�cula ya fue cargada.
...
//Verifica si el campo subt�tulo es correcto.
...
int aux,index=0,i=0;
    ...
    while ((aux!=13)&&(aux!=27)){
        ...
        fflush(stdin);
        aux=getch();
        if ((aux!=13)&&(((aux==32)||(aux<123)&&(aux>64))||((aux>47)&&(aux<58)))&&(i<sinopsis_max-1)){
            arreglo[i]=(char)aux;
            printf("%c",arreglo[i]);
            i++;
        }else if((aux==8)&&(i>0)){
...
//Carga una Pel�cula a un Archivo.
...
//Verifica si el usuario desea cargar otra Pel�cula.
...
void ingresarPeliculas(char DB_peliculas[], nodoArbol* arbol){
    ...
    if (verificacionPelicula(DB_peliculas, nombre, anio) == 0) {
        ...
        arbol = insertar(arbol,crearNodo(p));
...
nodoArbol* aplicarFiltrosDisp(stPelicula criterio, int campFil[], nodoArbol* peliculasFiltradas, nodoArbol* arbol){
    ...
    while(arbol){
        ...
        aplicarFiltrosDisp(criterio, campFil, peliculasFiltradas, arbol->izq);
        aplicarFiltrosDisp(criterio, campFil, peliculasFiltradas, arbol->der);
    }
...
//Insertar una pel�cula en orden por G�nero.
...
//Ordena pel�culas por T�tulo o por G�nero.
...
//Genera un listado de las pel�culas
...
void listarPeliculasDisponibles(nodoArbol* arbol, int opc, stUsuario user){
    system("cls");
...
int escribirNombre(char arreglo[]){
int aux,i=0;
    while ((aux!=13)&&(aux!=27)){
        fflush(stdin);
        aux=getch();
        if ((aux!=13)&&(((aux<123)&&(aux>64))||((aux>47)&&(aux<58))||(aux==32))&&(i<nombre_max-1)){
            arreglo[i]=(char)aux;
            printf("%c",arreglo[i]);
            i++;
        }else if((aux==8)&&(i>0)){
...
stPelicula buscarPelicula(nodoArbol* arbol){
    ...
    switch(opcion){
        case 0:
            ...
            mostrarIngresarID();
            scanf("%i",&buscarId);
            nodoBusqueda = buscar(arbol,buscarId);
