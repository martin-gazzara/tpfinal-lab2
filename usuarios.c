...
// Recomendar peliculas segun los generos m�s vistos por el usuario
...
// Calcula la edad del usuario en base a su a�o de nacimiento paara ver si puede ver "x" pelicula
...
//Oculta caracteres al ingresar contrase�a
...
//Verifica si las contrase�as ingresadas son iguales.
int verificarPass(char pass1[],char pass2[]){
    return strcmp(pass1,pass2);
}

//Verifica si el username est� en uso.
...
// 0: correcto 1: Contrase�as incorrectas 2: Usuario existente
...
void iniciarSesion(int val, stCelda adl[], nodoArbol* arbol, char DB_usuarios[],char DB_peliculas[]){
    // Variables que adquirir�n los datos ingresados
    ...
    if (esc!=27){
        ...
        if (index > -1){                                   // Si id==0, el usuario no existe
            system("cls");
            gotoxy(0,0);
            if (comprobarPass(password,adl[index].usr.pass, adl[index].usr.vectorKey)==0){    // Se comprueba que la contrase�a ingresada sea la correcta
                ...
                if (pelicula.id==0){
                    ...
                    system("cls");
                        ...
                        if(resp==0){
                            ...
                            if(edad>=pelicula.pm){
                                verPelicula(usuarios[index],pelicula);          ///   verPelicula()  .. Puede ser una nueva funcion que ir� solo en grafica
                                system("cls");
...
void mostrarHistorial(stCelda usuario){
    ...
    if (!usuario.listaPelis){
        ...
        if (i==0){
            borrar Historial               /// Deber�a retornar NULL ??
        }
    }
